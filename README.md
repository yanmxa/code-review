...
> A code review agent for GitHub / GitLab pull requests, built on the [pi](https://github.com/earendil-works/pi) framework.
> 基于 [pi](https://github.com/earendil-works/pi) 框架的 GitHub / GitLab PR 代码评审 Agent。

给一个 PR 链接，产出分级的评审意见 —— 断网重启能续跑，预算超了自动降级，每条意见都能追溯到它的原始 prompt 和模型回复，密钥永远不会离开本机。
...
**评审进行中 — the run dashboard.** 左边是文件进度，右边是 agent 此刻在做什么，顶部一行始终是"花了多少 / 还剩多少 / 现在用哪个模型"。

```
⬢ yanmxa/pi-review-demo #1 Add cache eviction, session lookup, and retry helper
...
**结果分诊 — the findings browser.** 按置信度分成两组，可采纳的默认已勾选，`p` 一键回评到 PR。右侧显示这条意见**凭什么**被判为可采纳。

```
⬢ Review findings · 9 total  ● 5  ○ 4                                         ▰▱▱▱▱▱▱▱▱▱ ¥0.41/¥6.00

╭─ Findings ─────────────────────── 5/9 ─╮╭─ Detail ───────────────────────────────────────────────╮
│● ADOPTABLE (5)                         ││● 新增了 console 日志                                   │
│▌[x] ● cache.ts:15 新增了 console 日志  ││F-001 · minor · adoptable                               │
│ [x] ● config.ts:4 提交中包含疑似密钥   ││src/cache.ts:15                                         │
│ [x] ● session.ts:14 SQL 语句拼接变量   ││                                                        │
│ [x] ● session.ts:8 用非密码学随机...   ││新增的 `console.log`/`console.debug`                    │
│ [x] ● session.ts:4 使用了宽松相等比较  ││通常是调试残留。若确实需要日志，请使用项目的 logger。   │
│                                        ││                                                        │
│○ REFERENCE (4)                         ││Evidence                                                │
│ [ ] ○ cache.ts:11 满容量时覆盖已有...  ││  ● rule console-log matched src/cache.ts:15 —          │
│ [ ] ○ cache.ts:12 当前驱逐策略按插...  ││    console.log("cache set", key);                      │
│ [ ] ○ retry.ts:3 重试次数比 `attem...  ││                                                        │
│ [ ] ○ session.ts:15 吞掉数据库异常...  ││→ traces/src_cache.ts.jsonl   t open trace              │
╰────────────────────────────────────────╯╰────────────────────────────────────────────────────────╯

                       ↑↓ move · space toggle · a all adoptable · t trace · p post · l lang · q quit
```

**追溯 — the trace viewer** (`t`). 每条意见背后完整的一条时间线：调了哪些工具、发出去的 prompt 原文、模型的原始回复、这一步花了多少钱。回车展开任意一行。
...
| 要求 | 怎么做的 | 代码 | 测试 |
| --- | --- | --- | --- |
| **可恢复**<br>断网/重启不从头跑 | Run id = `sha256(平台:仓库:PR号:head SHA)`，所以**重跑同一条命令就是续跑**，不需要记 run id。findings 先落盘、再写确认它的 state —— 崩在中间最多重跑一个文件，永远不会丢已付费的结果。`state.json` 用临时文件 + rename 原子写入。 | `src/checkpoint/store.ts` | `test/store.test.ts`（16）<br>`test/pipeline.test.ts` |
...
| **可观测**<br>每条评论关联 trace | 每个评审单元一个 JSONL 文件，记录完整 system prompt、每条消息、模型原始回复、每次工具调用与结果、规则命中、预算事件。每条 finding 带 `tracePath`，报告里是链接，TUI 里按 `t` 打开，命令行 `pi-review trace <run> <unit>`。 | `src/trace/tracer.ts` | `test/pipeline.test.ts`<br>`test/tui.test.ts` |
| **置信度分级**<br>可采纳 vs 仅供参考 | **只有机器可复现的证据才能评为"可直接采纳"**：确定性规则命中，或静态检查工具的诊断。模型自己的推理无论多笃定都只是"仅供参考"。模型必须在 `supportingToolCalls` 里引用工具调用 id，引用不存在的 id 会被静默丢弃而不是被奖励。 | `src/engine/grade.ts`<br>`src/engine/rules-engine.ts` | `test/rules.test.ts`（23）<br>`test/pipeline.test.ts` |
| **安全**<br>不上传 secret、不跑任意代码 | 脱敏是**编译器强制**的：所有出网/落盘的字符串都是 branded 类型 `Redacted<string>`，忘了脱敏是编译错误而不是泄漏。规则源自 gitleaks，外加熵值扫描。不 clone 仓库、不注册任何 shell 工具 —— 全程只有 REST 调用。唯一的子进程是 `gh auth token`。 | `src/security/redactor.ts`<br>`src/security/secret-rules.ts` | `test/redactor.test.ts`（27）<br>`test/post.test.ts` |
| **可扩展**<br>新增工具声明式注册 | 一个工具 = 一个文件导出 `ToolSpec` + `tools/index.ts` 里加一行。system prompt 的工具清单由 `meta.promptSnippet` 生成，置信度分级读 `meta.evidenceKind` —— 主流程一行都不用改。见下方 typecheck 的完整示例。 | `src/tools/spec.ts`<br>`src/tools/index.ts` | `test/tools.test.ts`（17） |

---

## 新增一个工具：完整示例 / Adding a tool
...
export const tsSyntaxCheckTool = defineReviewTool({
  meta: {
    ...
    costHint: "free",
    promptSnippet: "ts_syntax_check — 对改动的 .ts/.js 文件跑 TypeScript 编译器…",  // ← 自动进 system prompt
  },
  build(context) {
    return reviewTool({
...
**这个工具怎么保证安全**：文件通过 HTTPS 取到内存，交给 TypeScript 编译器的**虚拟 CompilerHost**。`noResolve` 让编译器不去碰 import，`noEmit` 让它不写任何文件 —— 编译器只是把仓库代码当**数据**来解析，不执行任何东西。代价是拿不到跨模块类型，所以"找不到模块"这类诊断会被过滤掉，只保留语法错误和文件内自洽的类型错误。这部分限制写在代码注释里，也写在 [设计文档](docs/design.zh.md) 里。
...
```bash
pi-review <pr-url> [options]      # 评审一个 PR / review a pull request
pi-review runs                    # 列出所有断点 / list checkpointed runs
pi-review triage <run-id>         # 重新打开某次运行的结果浏览器
pi-review trace <run-id> <unit>   # 打印某个单元的完整 trace
```
...
**配置优先级**：内置默认 → `~/.config/pi-review/config.json` → `./review.config.json` → 环境变量 → 命令行参数。
...
## 实际跑一次是什么样 / A real run

对着一个故意埋了缺陷的 PR（[yanmxa/pi-review-demo#1](https://github.com/yanmxa/pi-review-demo/pull/1)）：

```
$ pi-review https://github.com/yanmxa/pi-review-demo/pull/1 --budget 6 --no-tui

✓ src/cache.ts — 3 finding(s)
    ● minor  src/cache.ts:15   新增了 console 日志                    [可直接采纳]
...
```bash
npm test              # 182 个测试，全部离线（pi-ai 的 faux provider + 自建 Terminal 桩）
npm run typecheck
...
- **`ts_syntax_check` 拿不到跨模块类型**（没有 `node_modules`，见上文），只能查语法错误和文件内自洽的类型错误。
...
如果你在代理后面：Node 的 `fetch` 默认**不读** `HTTPS_PROXY`（curl 会读），表现出来是模型返回一个看起来像 API key 出问题的 `Connection error.`。pi-review 检测到配置了代理会自动带 `NODE_USE_ENV_PROXY=1` 重启一次，无需手动处理。

---

## 关于 AI 的使用 / On AI assistance
