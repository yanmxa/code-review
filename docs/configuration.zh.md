...
发现分两档，档位**只由证据决定**：命中规则或静态工具的诊断 → 「可直接采纳」，只有模型推理 → 「仅供参考」。

在「仅供参考」这一档内部，模型会额外交代自己有几分把握：`确定` / `多半` / `拿不准`。它**只影响组内排序**（严重级优先，其次才是把握程度），永远不会把一条发现抬进「可直接采纳」——否则两档分级就失去意义了。模型没填等同于「拿不准」：那个字段就是让它说明可信度的，沉默不算强主张。

「可直接采纳」的条目不显示自评：它们背后是任何人都能复现的证据，标一句「拿不准」只会让人困惑。

### rules

确定性检查，命中即为「可直接采纳」级证据。内置 11 条，`code-review config` 会全部列出。
