...
配置已存在时它就是编辑器：每个问题从文件里的现值开始，**它问不到的字段（`rules`、`review.focus`、`tools`）原样写回**。所以改预算或换模型都是再跑一次 `init`，不用记 JSON 长什么样。
...
`-y` 跳过全部提问（脚本用）。非 TTY 时：没有配置就写个空的，已有配置则原样不动。
...
| `ignore` | 评审时忽略的话题。写进评审员指令，让它不再就这些出意见 |
